    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>{t.productDetailsPage.description}</CardTitle>
            <CardDescription>{t.productDetailsPage.descriptionDesc}</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-sm leading-6 text-gray-700 whitespace-pre-wrap">
              {product.description || t.productDetailsPage.noDescription}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex items-center justify-between"><span className="text-gray-500">{t.productDetailsPage.status}</span><Badge variant={product.status === 'published' ? 'default' : 'secondary'}>{statusLabel}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-gray-500">{t.productDetailsPage.visibility}</span><Badge variant="outline">{visibilityLabel}</Badge></div>
            <div className="flex items-center justify-between"><span className="text-gray-500">{t.productDetailsPage.price}</span><span>{product.price} {product.currency}</span></div>
            <div className="flex items-center justify-between"><span className="text-gray-500">{t.productDetailsPage.salePrice}</span><span>{product.sale_price ?? '-'}</span></div>
