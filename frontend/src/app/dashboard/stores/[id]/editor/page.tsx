...
import { ArrowLeft, Check, Eye, Paintbrush, Palette, Save, Send, Store, Type, Upload, X } from "lucide-react";
...
export default function StoreBuilderPage() {
  ...
  const initialData = useMemo(
    () => parsePuckData(store?.storefront_layout_draft) || getDefaultPuckData(lang),
    [lang, store?.storefront_layout_draft],
  );
  const publishedData = useMemo(
    () => parsePuckData(store?.storefront_layout_published) || getDefaultPuckData(lang),
    [lang, store?.storefront_layout_published],
  );
  ...
  const hasUnsavedLayoutChanges = useMemo(
    () => serializeData(effectiveCurrentData) !== serializeData(initialData),
    [effectiveCurrentData, initialData],
  );

  const hasUnpublishedLayoutChanges = useMemo(
    () => serializeData(initialData) !== serializeData(publishedData),
    [initialData, publishedData],
  );

  const persistDraft = useCallback(
    async (layoutData: Data) => {
      if (!id) return;
  ...
  }, [effectiveCurrentData, id, persistDraft, t.storeEditorPage.draftSaveFailed, t.storeEditorPage.draftSaved]);
  ...
  const handlePublish = useCallback(
    ...
    [id, persistDraft, publishMutation, t.storeEditorPage.publishFailed, t.storeEditorPage.published],
  );
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50">
      <div className="border-b border-slate-200 bg-white px-4 py-4 shrink-0 z-50">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            >
              <ArrowLeft className="w-4 h-4" /> {t.storeEditorPage.backToStore}
            </Link>
            <div className="flex items-center gap-2 text-slate-900">
              <Store className="h-5 w-5 text-blue-600" />
              <h1 className="text-xl font-semibold">
                {store.name || t.storeEditorPage.storeFallback} · {t.storeEditorPage.title}
              </h1>
            </div>
            <p className="text-sm text-slate-600">{t.storeEditorPage.subtitle}</p>
          </div>

          <div className="flex items-center gap-2 flex-wrap">
            <Button type="button" variant="outline" onClick={() => void saveDraft()} disabled={updateMutation.isPending || publishMutation.isPending}>
              <Save className="mr-2 h-4 w-4" />
              {t.storeEditorPage.saveDraft}
            </Button>
            <Button type="button" onClick={() => void handlePublish(effectiveCurrentData)} disabled={updateMutation.isPending || publishMutation.isPending}>
              <Send className="mr-2 h-4 w-4" />
              {t.storeEditorPage.publishAll}
            </Button>
            <Button type="button" variant="secondary" asChild>
              <Link href={`/dashboard/stores/${id}/storefront?preview=true`} target="_blank">
                <Eye className="mr-2 h-4 w-4" />
                {t.storeEditorPage.preview}
              </Link>
            </Button>
            {status ? (
              <span className={`rounded px-2 py-1 text-xs font-medium ${status.type === "success" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>
                {status.text}
              </span>
            ) : null}
          </div>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="min-w-0 min-h-0 overflow-auto bg-white xl:border-r xl:border-gray-200 [&_.puck-Root]:h-full [&_.puck-Root]:flex [&_.puck-Root]:flex-col">
          <Puck
            key={store.id}
            config={editorConfig}
            data={initialData}
        ...
        <aside className="overflow-y-auto border-t border-gray-200 bg-slate-50 p-4 xl:border-l xl:border-t-0 space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t.storeEditorPage.draftStatusDesc}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-slate-600">
              <div className={`rounded-lg border px-3 py-2 ${hasUnsavedLayoutChanges || hasUnsavedThemeChanges ? "border-amber-200 bg-amber-50 text-amber-900" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>
                {hasUnsavedLayoutChanges || hasUnsavedThemeChanges ? t.storeEditorPage.unsavedChanges : t.storeEditorPage.allChangesSaved}
              </div>
              <div className={`rounded-lg border px-3 py-2 ${hasUnpublishedLayoutChanges ? "border-blue-200 bg-blue-50 text-blue-900" : "border-slate-200 bg-white text-slate-700"}`}>
                {hasUnpublishedLayoutChanges ? t.storeEditorPage.unpublishedChanges : t.storeEditorPage.publishedMatchesDraft}
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                ...
                {effectiveLogo ? (
                  <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-2">
                    <img src={effectiveLogo.startsWith("/") ? `http://localhost:8000${effectiveLogo}` : effectiveLogo} alt="Logo" className="h-10 w-10 rounded object-contain" />
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <input id="builder-primary" type="color" value={normalizedPrimary} onChange={(event) => setPrimaryColor(event.target.value)} className="h-9 w-10 rounded border" />
                    <Input value={effectivePrimaryColor} onChange={(event) => setPrimaryColor(event.target.value)} className="font-mono text-xs" />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="builder-secondary">{t.storeEditorPage.secondaryColor}</Label>
                  <div className="flex items-center gap-2">
                    <input id="builder-secondary" type="color" value={normalizedSecondary} onChange={(event) => setSecondaryColor(event.target.value)} className="h-9 w-10 rounded border" />
                    <Input value={effectiveSecondaryColor} onChange={(event) => setSecondaryColor(event.target.value)} className="font-mono text-xs" />
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(["light", "dark", "auto"] as const).map((mode) => (
                    <Button key={mode} type="button" variant={effectiveThemeMode === mode ? "default" : "outline"} onClick={() => setThemeMode(mode)}>
                      {mode === "light"
                        ? t.storeEditorPage.themeModeLight
                        : mode === "dark"
                        ? t.storeEditorPage.themeModeDark
                        : t.storeEditorPage.themeModeAuto}
                    </Button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input id="builder-font" value={effectiveFontFamily} onChange={(event) => setFontFamily(event.target.value)} list="builder-font-options" />
              ...
              <div className="rounded-lg border p-4" style={{ borderColor: normalizedSecondary, fontFamily: effectiveFontFamily }}>
                ...
                <div className="rounded-md p-4" style={{ backgroundColor: effectiveThemeMode === "dark" ? normalizedSecondary : "#ffffff" }}>
                  <p className="text-sm font-semibold" style={{ color: normalizedPrimary }}>{store.name || t.storeEditorPage.storeFallback}</p>
                ...
                {COLOR_PALETTES.map((pal) => {
                  ...
                  return (
                    <button
                      key={pal.key}
                      type="button"
                      className={`group relative flex items-center gap-2 rounded-lg border px-3 py-2 text-left text-xs font-medium transition-colors ${active ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50"}`}
                      onClick={() => {
                        setPrimaryColor(pal.primary);
                        setSecondaryColor(pal.secondary);
                        setThemeMode(pal.mode);
                      }}
                    >
                ...
                {TYPO_PRESETS.map((tp) => {
                  ...
                  return (
                    <button
                      key={tp.key}
                      type="button"
                      className={`group flex items-center justify-between rounded-lg border px-3 py-2.5 text-left transition-colors ${active ? "border-emerald-500 bg-emerald-50" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"}`}
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 truncate text-sm font-semibold text-slate-800" style={{ fontFamily: tp.font }}>
                          {store.name || "Store Name"}
                        </p>
                        <p className="mt-0.5 truncate text-xs text-slate-500" style={{ fontFamily: tp.font }}>
                          {tp.font}
                        </p>
                      </div>
